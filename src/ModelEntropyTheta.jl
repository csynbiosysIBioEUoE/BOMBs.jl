...
function genSamplesPrior(model_def, bayinf_def, nsamps, mu=[],coo=[])
    ...
    println("distributions with numbers defined in the parameters. If the parameters given to the ")
    ...
    println("")
    println("Also, sampling from all types of stan distributions is not implemented yet. For now the ")
...
function computeHgain(prior, posterior, model_def, tag = "")
    ...
    println("")
    println("----------------------------------------- ENTROPY RESULTS -----------------------------------------")
    println("Entropy gain results are saved in the directory: ")
    println(string("    ",cudi, "\\Results\\", model_def["NameF"],"_",today(), ""))
    println("Under the name: ")
