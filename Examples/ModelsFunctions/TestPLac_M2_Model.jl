    ...
    using ODEInterfaceDiffEq
        ...
        function TestPLac_M2ODE!(du,u,p,t)
        ...
        function TestPLac_M2SteadyState(p,I)
        ...
        alp[1] = Cit_mrna = (0.5 + Vm1*(IPTG^2/(Km1^2+IPTG^2)))/d1*aTc;;
        ...
        function TestPLac_M2_solvecoupledODE(ts, p, sp, inputs, ivss, pre=[])
            ...
            y_al = TestPLac_M2SteadyState(pSS,ivss) # Calculation of initial guesses for steady state
            ...
            for q in collect(1:Nevents)
                ...
                if q == 1
                    prob = ODEProblem(TestPLac_M2ODE!,initialV,(ts[(sp[q]+1)],ts[sp[q+1]+1]),pSte);
                    part1 = solve(prob, CVODE_BDF(),reltol=1.0e-9,abstol=1.0e-9,saveat=1);
                else
                    prob = ODEProblem(TestPLac_M2ODE!,initialV,(ts[(sp[q]+1)],ts[sp[q+1]+1]),pSte);
                    part1 = solve(prob, CVODE_BDF(),reltol=1.0e-9,abstol=1.0e-9,saveat=1);
                end
        ...
        function TestPLac_M2_SolveAll(ts, pD, sp, inputs, ivss, samps, pre=[])
            ...
            for drawInd in collect(1:length(pD[:,1]))
                ...
                temp = TestPLac_M2_solvecoupledODE(ts, p, sp, inputs, ivss2, pre);
