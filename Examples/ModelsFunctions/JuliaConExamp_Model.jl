    ...
    using ODEInterfaceDiffEq
        ...
        function JuliaConExamp_solvecoupledODE(ts, p, sp, inputs, ivss, pre=[])
            ...
            for q in collect(1:Nevents)
                ...
                if q == 1
                    prob = ODEProblem(JuliaConExampODE!,initialV,(ts[(sp[q]+1)],ts[sp[q+1]+1]),pSte);
        ...
        function JuliaConExamp_SolveAll(ts, pD, sp, inputs, ivss, samps, pre=[])
            ...
            for drawInd in collect(1:length(pD[:,1]))
                ...
                temp = JuliaConExamp_solvecoupledODE(ts, p, sp, inputs, ivss2, pre);
